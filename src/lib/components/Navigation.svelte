...
<header class="bg-white/90 backdrop-blur-md shadow-lg border-b border-gray-200/50 sticky top-0 z-40">
    ...
    <div class="flex justify-between h-16">
      <div class="flex">
        ...
        <div class="flex-shrink-0 flex items-center">
          <a
            href="/"
            class="text-xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent hover:from-indigo-700 hover:to-purple-700 transition-all duration-300 ease-in-out transform hover:scale-105"
          >
            📚 Librería Arco Iris
          </a>
        </div>
          ...
          {#each NAVIGATION_ITEMS as item}
            {#if item.href === '/lista-utiles'}
              ...
              <div class="relative" data-utils-menu>
                <button
                  on:click={toggleUtilsMenu}
                  on:keydown={(e) => (e.key === 'Enter' || e.key === ' ') && (toggleUtilsMenu(), e.preventDefault())}
                  class="{NAVIGATION_BASE_CLASSES} {isActive(item.href)
                    ? NAVIGATION_ACTIVE_CLASSES
                    : NAVIGATION_INACTIVE_CLASSES} {isUtilsMenuOpen ? 'bg-indigo-50 border-indigo-400 text-indigo-700' : ''}"
                  aria-expanded={isUtilsMenuOpen}
                  aria-haspopup="true"
                >
                  <span class="flex items-center">
                    {item.name}
                    <span class="ml-2 px-2 py-0.5 text-xs rounded-full bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 font-medium">
                      Nuevo
                    </span>
                    <svg
                      class="ml-1 h-4 w-4 transition-transform duration-200 {isUtilsMenuOpen ? 'rotate-180' : ''}"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                ...
                {#if isUtilsMenuOpen}
                  ...
                  <div
                    class="{DROPDOWN_STYLES.CONTAINER}"
      ...
      <div class="flex items-center space-x-4">
        ...
        {#if currentUser}
          ...
          <div class="ml-4 relative" data-profile-menu>
            <button
              type="button"
              class="relative group focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 rounded-full transition-all duration-200 ease-in-out"
              id="user-menu-button"
            ...
            {#if isProfileMenuOpen}
              ...
              <div
                class="origin-top-right absolute right-0 mt-3 w-56 rounded-xl shadow-xl bg-white/95 backdrop-blur-sm border border-gray-200/50 ring-1 ring-black/5 focus:outline-none z-50"
                transition:slide={{ duration: 200 }}
                role="menu"
              ...
              >
                <div class="py-2">
                  ...
                  <a
                    ...
                    class="text-gray-700 block px-4 py-3 text-sm hover:bg-gradient-to-r hover:from-indigo-50 hover:to-purple-50 hover:text-indigo-700 transition-all duration-200 ease-in-out rounded-none"
                    role="menuitem"
                    tabindex="-1"
                    on:keydown={(e) => {
                      if (e.key === 'Enter' || e.key === ' ') {
                  ...
                  <button
                    ...
                    class="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gradient-to-r hover:from-red-50 hover:to-pink-50 hover:text-red-700 transition-all duration-200 ease-in-out rounded-none border-t border-gray-100"
                    role="menuitem"
                    tabindex="-1"
      ...
      {#each NAVIGATION_ITEMS as item}
        <a
          ...
          class="{BUTTON_STYLES.MOBILE} {isActive(item.href)
            ? 'bg-gradient-to-r from-indigo-50 to-purple-50 border-indigo-500 text-indigo-700'
            : 'border-transparent text-gray-600 hover:bg-gradient-to-r hover:from-gray-50 hover:to-gray-100 hover:border-gray-300 hover:text-gray-900'}"
        >
          <span class="flex items-center justify-between w-full">
            <span>{item.name}</span>
            {#if item.href === '/lista-utiles'}
              <span class="px-2 py-0.5 text-xs rounded-full bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-700 font-medium">
                Nuevo
              </span>
            {/if}
          </span>
  ...
  {#if notificationMessage}
    ...
    >
      <div class="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-lg shadow-lg p-4">
        <div class="flex items-center">
