<script lang="ts">
  ...
  function getIcon(type: string) {
    switch (type) {
      ...
      default: // info
        return 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h2a1 1 0 100-2v-3a1 1 0 00-1-1H9z';
    }
  }
...
{#if toasts.length > 0}
  <div class="fixed inset-0 flex items-start justify-end p-4 px-4 py-6 pointer-events-none sm:p-6 sm:items-start sm:justify-end z-50">
    <div class="w-full flex flex-col items-end space-y-4">
      {#each toasts as toast (toast.id)}
        <div
          class="max-w-sm w-full rounded-lg shadow-lg overflow-hidden pointer-events-auto"
          ...
          <div class="p-4">
            <div class="flex items-start">
              <div class="flex-shrink-0">
                <svg 
                  class={`h-5 w-5 ${getTextColor(toast.type)}`} 
                  viewBox="0 0 20 20" 
                  fill="currentColor"
                >
